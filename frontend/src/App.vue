...
const downloadAsset = (asset: AssetItem) => {
  ...
  link.target = "_blank";
  link.click();
  addNotification("success", "下载已开始", link.download);
};
          ...
          <article v-for="asset in assetItems" :key="`${asset.type}-${asset.id}`" class="asset-card">
            ...
            <div class="history-card-meta">
              ...
              <div class="page-controls">
                ...
                <button class="log-action" @click="downloadAsset(asset)">下载</button>
              </div>
            </div>
          </article>
          ...
          <article v-for="item in imageHistories" :key="String(item.id)" class="history-card">
            ...
            <div class="history-card-meta">
              ...
              <div class="page-controls">
                ...
                <button class="item-action-btn" @click="downloadAsset({ id: String(item.id), type: 'image', title: String(item.prompt || '历史图片'), subtitle: String(item.createdAt || ''), url: String(item.url || '') })">下载</button>
      ...
      <section v-else-if="activeView === 'ttsHistory'" class="module-page">
        ...
        <div class="module-body voice-history-list">
          <article v-for="item in ttsHistories" :key="String(item.id)" class="voice-history-card">
            ...
            <button class="item-action-btn" @click="downloadAsset({ id: String(item.id), type: 'audio', title: String(item.text || '历史语音'), subtitle: `${item.voiceId || 'voice'} · ${item.format || 'audio'} · ${item.preview ? 'preview' : 'tts'}`, url: String(item.audioUrl || '') })">下载</button>
...
.voice-history-card { display: grid; grid-template-columns: minmax(220px, 1fr) minmax(260px, 480px) auto auto; align-items: center; gap: 12px; border: 1px solid var(--border); border-radius: 8px; background: rgba(18,18,18,0.88); padding: 10px; }
